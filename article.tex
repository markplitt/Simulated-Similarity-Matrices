...
\title{Mark's attempt to use LaTeX to make it easier to write equations to send Kiah}

\author[a]{Mark H. Plitt}
\affil[a]{Mark's House in the Woods, Department of Shenanigans, Woodside, CA}
...
\section{Creating a population that will robustly encode a (univariate) distribution}
\label{sect:intro}  % \label{} allows reference to this section
Let's assume that upstream of our population of interest, there is a magical process that spits out samples from an arbitrary univariate distribution of stimulus values. We will refer to our downstream population as $\vec{y}$, and to value of each sample from the distribution, $D$, as $s$, where $s \in \mathbb{R}$, $\vec{y} \in \mathbb{R}^{n}$, and $n$ is the number of neurons in the population. We will call each instance of $s$ as a "trial". Trials will be single timepoints, i.e. neurons will not have any dynamics.

Each neuron's activity on a given trial will be governed by a gamma distribution $P(y_i; \kappa, \theta) = \frac{y_i^{\kappa -1}}{\Gamma (\kappa) \theta^\kappa} e^{\frac{y_i}{\theta} } $. A gamma distribution was chosen as a continuous analog of a Poisson distribution. To make this analogy even closer, we will fix $\theta$ to have a value of $1$ . This makes the mean and variance of the gamma distribution both $\kappa$. In order to get $\vec{y}$ to encode $s$, we will let $\kappa$ be a function of the $s$ and allow it to be specific to each output neuron, denoted $\kappa_i(s)$. 

A simple way to get $\vec{y}$ to encode $s$ is to make $\kappa_i(s)$ a set of generic basis functions. For simplicity, we will use radial basis functions,  $\kappa_i(s; \sigma, \mu_i) = exp(\frac{(s-\mu_i)^2}{\sigma^2})$.

Thus the activity of neuron $i$ on trial $j$ is random variable that depends on $s_j$ and is given by 
$$ y_i(s_j) \sim gamma(s_j; \kappa_i(s_j)) + gamma(\epsilon)$$

The $gamma(\epsilon)$ term is included to reduce to the sparsity of the representation across the population. 

In expectation, with sufficiently small $\sigma$, we would expect this population's average activity to be proportional to the $D$, the distribution from which $s$ is drawn. 

We will not prove that this is generically true, but we will demonstrate that it is empirically true in this application and that the $Maximum a Posteriori$ estimate from this population is the same as estimate $D$. In addition we will demonstrate that the $MAP$ estimate can be inferred from a linear function of the low dimensional geometry of the encodings for $s$. 
